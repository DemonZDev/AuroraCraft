...
// Everything a metered NIM feature needs to route over a key chain AND bill each call.
// For free providers `pricing` is zero and `keys` holds the single key (no real billing).
export interface NimBilling {
  userId: string
  modelId: string
  providerSlug: string
...
// Per-call routing for the NIM features (lighter than the agent's LiteLLM path; these
// are short, non-streaming calls). Mirrors routing.md §7 rules at a smaller scale.
const NIM_RETRIES_PER_KEY = 2     // retry the current key on transient failure
const NIM_RETRY_DELAY_MS = 3000   // delay between retries
const NIM_MAX_SWEEPS = 2          // full-chain sweeps before giving up

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms))

// One metered NIM call: route over the key chain (retry → fall back → loop), and on
// success charge BOTH the user's tokens and the serving key's dollar limit in real time.
// Throws BalanceExhaustedError when the user runs out, or the last NimError when the
// whole chain fails. Free providers skip dollar billing (pricing is zero).
...
async function meteredNimChat(
  billing: NimBilling,
  baseUrl: string,
  realName: string,
  opts: { messages: NimMessage[]; maxTokens: number; signal?: AbortSignal; sessionId?: string },
): Promise<{ content: string; finishReason: string | null }> {
  const keys = billing.keys.length ? billing.keys : [{ id: '', apiKey: '', weight: 0, limitUsd: null, usedUsd: 0, remainingUsd: null }]

  let lastErr: unknown
  for (let sweep = 0; sweep < NIM_MAX_SWEEPS; sweep++) {
    for (const key of keys) {
      for (let attempt = 0; attempt <= NIM_RETRIES_PER_KEY; attempt++) {
        if (opts.signal?.aborted) throw new NimError('NIM request aborted', undefined, true)
        try {
          ...
          if (err instanceof NimError && err.aborted) throw err
          lastErr = err
          if (attempt < NIM_RETRIES_PER_KEY) await sleep(NIM_RETRY_DELAY_MS)
        }
      }
      // exhausted this key's retries → fall back to next key in the chain
    }
  }
  throw lastErr ?? new NimError('All API keys failed')
}
...
class NimEngine {
  ...
  // ── Prompt Enhancer ────────────────────────────────────────────────────
  async runEnhance(jobId: string, billing: NimBilling, style: EnhanceStyle, target: NimTarget, input: EnhanceInput): Promise<void> {
    ...
    try {
      ...
      const draft = await meteredNimChat(billing, target.baseUrl, target.slug, {
      ...
      const finalized = await meteredNimChat(billing, target.baseUrl, target.slug, {
        maxTokens: ENHANCE_MAX_TOKENS, signal,
        messages: [
  ...
  async runRefine(jobId: string, billing: NimBilling, target: NimTarget, currentPrompt: string, changeRequest: string, history: unknown[]): Promise<void> {
    const signal = this.register(jobId)
    try {
      const out = await meteredNimChat(billing, target.baseUrl, target.slug, {
        maxTokens: ENHANCE_MAX_TOKENS, signal,
        messages: [
  ...
  async runErrorFix(jobId: string, billing: NimBilling, target: NimTarget, input: ErrorFixInput): Promise<void> {
    const signal = this.register(jobId)
    try {
      ...
      const out = await meteredNimChat(billing, target.baseUrl, target.slug, { maxTokens: FIX_MAX_TOKENS, signal, messages })
