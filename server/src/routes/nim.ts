...
import { nimEngine, type EnhanceStyle } from '../agents/nim-engine.js'
...
export async function nimRoutes(app: FastifyInstance) {
  ...
  async function requireNimKey(request: FastifyRequest, reply: FastifyReply): Promise<string | null> {
  ...
  // ── Models (for the pickers) ──────────────────────────────────────────
  app.get('/api/nim/models', { preHandler: [authMiddleware] }, async () => {
    return {
      models: NIM_MODELS.map(({ id, label, isReasoning }) => ({ id, label, isReasoning })),
      defaultId: DEFAULT_NIM_MODEL_ID,
    }
  })
  ...
  app.post('/api/projects/:id/nim/enhance', { preHandler: [authMiddleware] }, async (request, reply) => {
    ...
    void nimEngine.runEnhance(job.id, key, parsed.data.style as EnhanceStyle, modelId, {
